...
function drawHitMapHex() {
    ...
    return function updateChart() {
        const binned = hexbin(data.map(d => [d[0] * width / 2, d[1] * width / 2])), maxsize = d3.max(binned.map(d => d.length)), size = d3.scaleLinear().domain([0, maxsize]).range([0, binsize]), opacity = d3.scaleLinear().domain([0, maxsize]).range([0, 1]), heatmap = g.selectAll('.bin').data(binned);
        heatmap.exit().remove();
...
//# sourceMappingURL=hypersphere.js.map